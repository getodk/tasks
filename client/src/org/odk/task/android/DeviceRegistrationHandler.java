...
import org.odk.task.android.comm.SmsSender;

import java.util.HashMap;
import java.util.Map;

/**
 * This handler is responsible for carrying out device registration with the ODK
 * Manage server.
 * 
...
public class DeviceRegistrationHandler {
	...
	/**
	 * 
	 * @return true if the IMSI has changed since the last registration. Returns
	 *         false if there is a registration SMS in transit.
	 */
	public boolean registrationNeededForImei() {
		boolean registrationInProgress = prefsAdapter.getBoolean(Constants.PREFS_REG_IN_PROGRESS_KEY, false);
	...
	public void registerBySms(boolean returnToast) {
		...
		new SmsSender(ctx).sendSMS(prefsAdapter.getString(Constants.PREF_SMS_KEY, null), Constants.SMS_REGISTER_ACTION,
				regMap, onSent, onDelivered);

	}

	/**
	 * Attempts to register the device with the server. TODO(alerer): should be
	 * done asynchronously.
	 * 
	 * @param returnToast
	 */
	...
	private Map<String, String> createRegisterMap() {
		...
		newProperty("userid", prefsAdapter.getString(Constants.PREF_USERID_KEY, ""), paramMap);
		...
		newProperty("userName", prefsAdapter.getString(Constants.PREF_NAME_KEY, ""), paramMap);
		newProperty("userLocation", prefsAdapter.getString(Constants.PREF_LOCATION_KEY, ""), paramMap);
		return paramMap;
	}
